...
| [`backends`](enlace_auth.stores.backends.md#module-enlace_auth.stores.backends)     | MutableMapping-backed store factories for enlace.                       |
|--------------------------------------------------------------------------------------------------|-------------------------------------------------------------------------|
| [`middleware`](enlace_auth.stores.middleware.md#module-enlace_auth.stores.middleware) | Store injection middleware and per-app store router.                    |
| [`prefixed`](enlace_auth.stores.prefixed.md#module-enlace_auth.stores.prefixed)     | PrefixedStore — MutableMapping wrapper that scopes keys under a prefix. |
| [`validation`](enlace_auth.stores.validation.md#module-enlace_auth.stores.validation) | Key sanitization for user-supplied store keys.                          |
